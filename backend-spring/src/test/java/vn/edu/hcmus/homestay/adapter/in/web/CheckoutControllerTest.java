...
class CheckoutControllerTest {
    ...
    @MockitoBean
    private ManageSettlementUseCase manageSettlementUseCase;

    @MockitoBean
    private ManageInspectionUseCase manageInspectionUseCase;
    ...
    void getSettlement_authenticated_200() throws Exception {
        ...
        UUID requestId = UUID.randomUUID();
        UserPrincipal principal = new UserPrincipal(customerId, "customer@example.com", AppRole.CUSTOMER);
        when(manageSettlementUseCase.getSettlement(eq(requestId))).thenReturn(settlement(requestId));
    ...
    void createSettlement_asAccountant_201() throws Exception {
        ...
        when(manageSettlementUseCase.createSettlement(eq(requestId), any())).thenReturn(settlement(requestId));
