...
@Component({
  ...
  template: `
        ...
        <div style="width:1920px;height:1080px;position:relative;background:#FEF4DF;overflow:hidden">
          ...
          <div style="position:absolute;left:510px;top:340px;width:1295px;height:625px;display:flex;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08)">
            ...
            <div style="width:320px;flex-shrink:0;background:white;border-right:1px solid #e2e8f0;display:flex;flex-direction:column;overflow-y:auto">
              <div *ngIf="loadingConversations" style="padding:40px;text-align:center;color:#94a3b8;font-size:18px;font-family:Afacad">Loading…</div>
              <div *ngIf="!loadingConversations && conversations.length===0" style="padding:40px;text-align:center;color:#94a3b8;font-size:18px;font-family:Afacad">No conversations yet</div>
              <button *ngFor="let conv of conversations" class="conv-btn" [class.active]="selectedId===conv.id" (click)="selectConversation(conv)">
                <div style="display:flex;justify-content:space-between;align-items:center">
                  ...
                  <span style="font-size:12px;border-radius:999px;padding:2px 8px;font-family:Afacad" [style.background]="selectedId===conv.id?'rgba(255,255,255,.2)':(conv.status==='open'?'#dcfce7':'#f1f5f9')" [style.color]="selectedId===conv.id?'white':(conv.status==='open'?'#16a34a':'#64748b')">{{conv.status==='open'?'Open':'Closed'}}</span>
                </div>
            ...
            <div style="flex:1;display:flex;flex-direction:column;background:#f8fafc;overflow:hidden">
              ...
              <div *ngIf="!selectedConversation" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:#94a3b8">
                ...
                <p style="font-size:18px;font-family:Afacad">Select a conversation to start chatting</p>
              </div>
              ...
              <ng-container *ngIf="selectedConversation">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e2e8f0;background:white;padding:14px 20px;flex-shrink:0">
                  ...
                  <button *ngIf="selectedConversation.status==='open'" style="background:#e2e8f0;border:none;border-radius:8px;padding:6px 16px;font-size:16px;cursor:pointer;color:#475569;font-family:Afacad" [disabled]="isClosing" (click)="closeConversation()">{{isClosing?'Closing…':'Close'}}</button>
                  <span *ngIf="selectedConversation.status!=='open'" style="background:#f1f5f9;border-radius:8px;padding:6px 16px;font-size:16px;color:#94a3b8;font-family:Afacad">Closed</span>
                </div>

                <!-- Messages -->
                <div style="flex:1;overflow-y:auto;padding:16px 20px;display:flex;flex-direction:column;gap:10px" #msgsEl>
                  <div *ngIf="loadingMessages" style="text-align:center;color:#94a3b8;font-size:16px;font-family:Afacad;padding:30px">Loading…</div>
                  <p *ngIf="!loadingMessages && messages.length===0" style="text-align:center;color:#94a3b8;font-size:16px;font-family:Afacad;margin-top:30px">No messages yet.</p>
                ...
                <div *ngIf="selectedConversation.status==='open'" style="display:flex;align-items:center;gap:10px;border-top:1px solid #e2e8f0;background:white;padding:12px 16px;flex-shrink:0">
                  <input style="flex:1;border-radius:999px;border:1px solid #e2e8f0;background:#f8fafc;padding:8px 18px;font-size:16px;outline:none;font-family:Afacad" [(ngModel)]="draft" placeholder="Type a reply…" (keydown.enter)="send()" autocomplete="off" [disabled]="isSending"/>
    ...
    <ng-template #sidebarAndMenus>
      ...
      <!-- Sidebar background (replaces ChatUnion.png with precise CSS) -->
      <div style="width:405px;height:1080px;left:0;top:0;position:absolute;background:#264893;"></div>
      <!-- Active white pill at Chat item position -->
      <div style="width:323px;height:54px;left:82px;top:762px;position:absolute;background:white;border-radius:27px;"></div>
      ...
      <div (click)="navigate('/admin/rental-requests')" class="hover-effect" style="cursor:pointer;width:196px;height:54.75px;left:166px;top:331px;position:absolute;justify-content:center;display:flex;flex-direction:column;color:#FEF4DF;font-size:32px;font-family:Afacad;font-weight:500;word-wrap:break-word">{{ "ADMIN_RENTAL.SIDEBAR.INQUIRIES" | translate }}</div>
      ...
      <div (click)="navigate('/admin/scheduled')" class="hover-effect" style="cursor:pointer;width:126px;height:54.75px;left:166px;top:417.54px;position:absolute;justify-content:center;display:flex;flex-direction:column;color:#FEF4DF;font-size:32px;font-family:Afacad;font-weight:500;word-wrap:break-word">{{ "ADMIN_RENTAL.SIDEBAR.SCHEDULES" | translate }}</div>
      ...
      <div (click)="navigate('/admin/rooms')" class="hover-effect" style="cursor:pointer;width:195px;height:54.75px;left:161px;top:504.07px;position:absolute;justify-content:center;display:flex;flex-direction:column;color:#FEF4DF;font-size:32px;font-family:Afacad;font-weight:500;word-wrap:break-word">{{ "ADMIN_RENTAL.SIDEBAR.ROOMS" | translate }}</div>
      ...
      <div (click)="navigate('/admin/payments')" class="hover-effect" style="cursor:pointer;width:175px;height:54.75px;left:166px;top:589.72px;position:absolute;justify-content:center;display:flex;flex-direction:column;color:#FEF4DF;font-size:32px;font-family:Afacad;font-weight:500;word-wrap:break-word">{{ "ADMIN_RENTAL.SIDEBAR.RESERVATIONS" | translate }}</div>
      ...
      <div (click)="navigate('/admin/users')" class="hover-effect" style="cursor:pointer;width:168px;height:54.75px;left:163px;top:676.25px;position:absolute;justify-content:center;display:flex;flex-direction:column;color:#FEF4DF;font-size:32px;font-family:Afacad;font-weight:500;word-wrap:break-word">{{ "ADMIN_RENTAL.SIDEBAR.CONTRACTS" | translate }}</div>
      ...
      <div (click)="navigate('/admin/chat')" class="hover-effect" style="cursor:pointer;width:168px;height:54.75px;left:163px;top:762px;position:absolute;justify-content:center;display:flex;flex-direction:column;color:#264893;font-size:32px;font-family:Afacad;font-weight:700;word-wrap:break-word">{{ "ADMIN_RENTAL.SIDEBAR.CHAT" | translate }}</div>
